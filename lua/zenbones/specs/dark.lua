...
local function generate(p, opt)
	...
	local base = lush(function()
		return {
			...
			Comment         { fg = Normal.bg.li(opt.lighten_comments or 38).de(24), gui = opt.italic_comments ~= false and "italic" or "NONE" }, -- any comment
			...
			LineNr          { fg = Normal.bg.li(opt.lighten_line_nr or 35), bg = opt.solid_line_nr and Normal.bg.li(6) or "NONE" }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
			...
			StatusLine      { bg = Normal.bg.li(16), fg = p.fg }, -- status line of current window
			StatusLineNC    { bg = Normal.bg.li(11), fg = p.fg.li(28) }, -- status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
			...
			Type            { fg = Normal.bg.li(58) }, -- (preferred) int, long, char, etc.
			...
			IndentBlanklineChar              { fg = Normal.bg.li(14).de(22) },
